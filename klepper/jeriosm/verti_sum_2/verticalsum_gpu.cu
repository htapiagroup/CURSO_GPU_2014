...
extern "C" void main_gpu (float *d_odata, float *d_idata, int size_x, int size_y) {
  ...
  config.options = CUDPP_OPTION_EXCLUSIVE | CUDPP_OPTION_FORWARD; //AQUI: lea la parte CPU y complete las opciones
  ...
  cudaMallocPitch(&tr_in, &pitch_tr_in, size_y*sizeof(float), pitch/sizeof(float));
  cudaMallocPitch(&tr_in_sum, &pitch_tr_in, size_y*sizeof(float), pitch/sizeof(float));
  cudaMallocPitch(&tr_out, &pitch_tr_out, pitch, pitch_tr_in/sizeof(float));
  ...
  // AQUI: transponga la matrix d_idata en tr_in
   transpose(d_idata, tr_in, pitch/sizeof(float), pitch_tr_in/sizeof(float));

  // AQUI: disenye el plan 'scanplan' para el multiscan
   res = cudppPlan(theCudpp, &scanplan, config, size_y, size_x, pitch_tr_in/sizeof(float));
  ...
  cudppMultiScan(scanplan, tr_in_sum, tr_in, size_y, size_x);
  ...
  // AQUI: tranponga el resultado de tr_in_sum en tr_out
    transpose(tr_in_sum, tr_out, pitch_tr_in/sizeof(float), pitch/sizeof(float));

  // AQUI: copie tr_out en d_odata en el device
   cudaMemcpy2D(d_odata, pitch, tr_out, pitch, size_x*sizeof(float), size_y*sizeof(float), cudaMemcpyDeviceToDevice);
}
