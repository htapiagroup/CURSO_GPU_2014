...
__global__ void kconvol (float *gpu_a, float *gpu_b, int pitch, int n) {
  ...
  if ((i == 0) || (j == 0) || (i == n-1) || (j == n-1))  {
    ...
    gpu_b[l]=(1.f/5.f)*(                 gpu_a[l-pitch]+\
			gpu_a[l-1/*MISSING*/]  +gpu_a[l]         +gpu_a[l+1]+ \
			                 gpu_a[l+pitch/*MISSING*/]                 );
}
...
extern "C" void gpu_convol (float *a, float *b, int n) {
  ...
  grid.x = (n+BLOCK_X-1)/BLOCK_X;
  grid.y = (n+BLOCK_Y-1)/BLOCK_Y;
  ...
  kconvol <<<grid, block>>> (gpu_a, gpu_b,pitch/sizeof(float) /* MISSING */, n);
